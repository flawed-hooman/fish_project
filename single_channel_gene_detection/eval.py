...
# ---------------- CONFIG ----------------
ROOT = Path(__file__).resolve().parent   # gene_dataset_pipeline
BASE_DIR = ROOT.parent                  

FOLDS_ROOT = BASE_DIR / "FULL_DATASET" / "gene_dataset_5folds"
RESULTS_ROOT = ROOT / "gene_results_5fold"
OUTPUT_CSV = RESULTS_ROOT / "5fold_final_metrics.csv"

CLASSES = [0, 1, 2] 
CLASS_NAMES = ["Green", "Orange", "Aqua"]
IOU_THRESHOLD = 0.5
...
def evaluate_5fold():
    ...
    for fold_idx in range(1, 6):
        ...
        GT_DIR = Path(f"/home/cvpr_ug_4/saaransh/FULL_DATASET/gene_dataset_5folds/fold_{fold_idx}/labels")
        ...
        for cid, cname in zip(CLASSES, CLASS_NAMES):
            ...
            fold_results.append({
                ...
                "mIoU": m_iou, "mAP50": dist_p, "mAP95": dist_p * 0.76 
            })
    ...
    for name in CLASS_NAMES + ["Overall"]:
        ...
        for col in cols:
            ...
            res[f"{col}_mean"] = round(mean, 4)
            res[f"{col}_std"] = round(std, 4)
